...
static struct btf *btfgen_get_btf(struct btfgen_info *info)
{
	...
	hashmap__for_each_entry(info->types, entry, bkt) {
		...
		const struct btf_type *btf_type = btfgen_type->type;
		int new_id;
		...
		if (btf_is_struct(btf_type) || btf_is_union(btf_type)) {
			...
			/* copy btf type */
			btf_type_cpy->name_off = btf_type->name_off;
			btf_type_cpy->info = btf_type->info;
			btf_type_cpy->type = btf_type->type; // type or size
			...
			if (nmembers > 0) {
				...
				hashmap__for_each_entry(btfgen_type->members, member_entry, bkt2) {
					...
					btf_member_src = btf_members(btf_type) + btfgen_member->idx;
					btf_member_dst = btf_members(btf_type_cpy) + index;

					btf_member_dst->name_off = btf_member_src->name_off;
					btf_member_dst->type = btf_member_src->type;
					btf_member_dst->offset = btf_member_src->offset;
...
static int do_min_core_btf(int argc, char **argv)
{
	...
	if (input_is_file) {
		...
		snprintf(dst_btf_path, sizeof(dst_btf_path), "%s/%s", output,
			 basename(input));
		err = minimize_btf(input, dst_btf_path, objs);
		goto out;
	}
	...
	while ((dir = readdir(d)) != NULL) {
		...
		err = minimize_btf(src_btf_path, dst_btf_path, objs);
		if (err)
			goto out;
	}
